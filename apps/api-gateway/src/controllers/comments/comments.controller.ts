...
import { CreateCommentResponse, SearchCommentResponse } from './types';
...
export class CommentsController {
	...
	@ApiCreatedResponse({description: 'Comentário feito com sucesso', type: CreateCommentResponse})
	...
	@ApiOkResponse({description: 'Lista de comentários retornada', type: SearchCommentResponse})
	async search(@Param('id') id: string, @Query() params: Pagination) {
		...
		return await this.taskProxy.send<SearchCommentResponse, SearchCommentReq>('comments-list', data, options);
	}
}
